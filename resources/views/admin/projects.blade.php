		                ...
		                <div class="table-responsive  table-has-actions   table-has-filter ">
                            <div id="table-categories_wrapper" class="dataTables_wrapper container-fluid dt-bootstrap4 no-footer" style="width: 100%;">
                            	...
                            	<div class="dt-buttons btn-group"> 
                            		<button class="btn btn-secondary action-item" tabindex="0" aria-controls="table-categories"><span><span data-action="create" data-href="{{URL::route('addProject')}}"><i class="fa fa-plus"></i> Tạo mới</span></span></button> 
											...
											@endif
													<td class=" text-left no-sort">
														<div class="text-left">
														    <div class="checkbox checkbox-primary table-checkbox">
														        <input type="checkbox" class="checkboxes" name="id[]" value="{{$project->id}}">
														    </div>
														</div>
													</td>
													<td class="column-key-id sorting_{{$i}}">{{$i}}</td>
													<td class="  column-key-image">
														<img src="{{asset('uploads/images/projects/avatas/'.$project->avata)}}" width="50" alt="{{$project->name}}">
													</td>
													<td class=" text-left column-key-name"><a href="">{{$project->name}}</a></td>
													@php
													...
													<td class=" no-sort column-key-updated_at">
														@foreach($pjcids as $pjcid)
															@php
																$c = App\ProjectCate::where('id',$pjcid->cate_id)->get()->first();
															@endphp
													...
													<td class="  column-key-created_at">{{$project->created_at}}</td>
													...
													<td class=" text-center">
														<div class="table-actions">
										                    ...
										                    <a href="{{URL::route('editProject',$project->id)}}" class="btn btn-icon btn-sm btn-primary" data-toggle="tooltip" data-original-title="Sửa">
										                    	<i class="fa fa-edit"></i>
										                    </a>
										        
										                    <a href="#" class="btn btn-icon btn-sm btn-danger deleteDialog delete" data-toggle="tooltip" data-section="{{URL::route('deleteProject',$project->id)}}" role="button" data-original-title="Xóa bản ghi">
